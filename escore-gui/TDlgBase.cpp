...
void TDlgBase::formReposition()
{
  ...
  if(
    mainFrm &&
    TFormPosition::MainFormCenter == Position
  )
  {
  ...
  if(
    !positioned &&
    TFormPosition::OwnerFormCenter == Position &&
    nullptr != Owner
  )
  {
    TCustomForm* ownerFrm = dynamic_cast<TCustomForm*>(Owner);
    if( ownerFrm )
    {
      bounds.Top = ownerFrm->Top;
      bounds.Left = ownerFrm->Left;
      bounds.SetWidth( ownerFrm->Width );
      bounds.SetHeight( ownerFrm->Height );
      positioned = true;
    }
  }
  ...
  if(
    !positioned &&
    (
      TFormPosition::Default == Position ||
      TFormPosition::DefaultPosOnly == Position ||
      TFormPosition::DefaultSizeOnly == Position ||
      TFormPosition::ScreenCenter == Position ||
      TFormPosition::DesktopCenter == Position
    )
  )
  {
