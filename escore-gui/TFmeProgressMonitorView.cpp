...
EsEventSubscriber(
  EsString::null(), //< Capture all event categories (progress tasks events may have arbitrary category)
  precedenceHigher  //< Elevated priority, to handle progress events before any other visual elements,
                    //  this would allow us to update realHeight properly before it's used in visual clients
),
m_this(This)
{
	ES_ASSERT(m_this);
...
void TFmeProgressMonitorView::heightUpdate()
{
  ...
  for( size_t idx = 0; idx < m_items.countGet(); ++idx )
  {
    ...
    float itemHeight = item->Height + 2*borderH;
