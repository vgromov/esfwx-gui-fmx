...
void EsReflectedClassPropertyLink::sourceSet(EsReflectedClassDataSource* src, ulong priority /*= 1000*/)
{
  if( m_src != src )
  {
    if( m_src )
...
void EsReflectedClassPropertyLink::updateControl(const EsReflectedClassIntf::Ptr& obj)
{
  ...
  m_canWrite = obj->propertyCanWrite(m_propName);
...
void EsReflectedClassDataSource::link(EsReflectedClassPropertyLink* link, ulong priority /*= 1000*/)
{
  ES_ASSERT(link);
  link->sourceSet(this, priority);
}
//---------------------------------------------------------------------------

void EsReflectedClassDataSource::linkAdd(EsReflectedClassPropertyLink* link, ulong priority)
{
  ES_ASSERT(link);
...
EsReflectedClassPropertyLink* EsReflectedClassDataSource::linkFind(const EsString& propName) const
{
  for( auto link : m_links )
    if( propName == link.second->propertyNameGet() )
      return link.second;
...
{
  for(auto it = m_links.begin(); it != m_links.end(); ++it)
    (*it).second->resetControlToDefault();

  if( m_onCtrlsUpdated )
    m_onCtrlsUpdated(this);
}
//---------------------------------------------------------------------------
...
void EsReflectedClassDataSource::updateObject(const EsReflectedClassIntf::Ptr& obj) const
{
  ...
  if( obj )
  {
    ...
    try
    {
      if( obj->hasMethod(EsMethodInfoKeyT(esT("propertiesSetBegin|0"))) )
        obj->call(esT("propertiesSetBegin"));

      for(auto it = m_links.begin(); it != m_links.end(); ++it)
        (*it).second->updateProperty(obj);

    }
    __finally
    {
      if( obj->hasMethod(EsMethodInfoKeyT(esT("propertiesSetEnd|0"))) )
        obj->call(esT("propertiesSetEnd"));
    }
  }
}
//---------------------------------------------------------------------------
...
///
__fastcall EsRttiPropertyIndexLink::EsRttiPropertyIndexLink(EsRttiPropertyLinkIndexed* container, ulong index, TComponent* cmp, const EsString& rttiProp) :
TComponent(cmp),
m_container(0),
m_cmp(cmp),
m_ptype(0),
...
__fastcall EsRttiPropertyIndexLink::~EsRttiPropertyIndexLink()
{
  containerSet(nullptr);
}
//---------------------------------------------------------------------------

ulong EsRttiPropertyIndexLink::indexGet() const
{
  return m_index;
}
//---------------------------------------------------------------------------

void EsRttiPropertyIndexLink::containerSet(EsRttiPropertyLinkIndexed* container)
{
...
void EsRttiPropertyIndexLink::doControlValSet(const EsVariant& val)
{
  ES_ASSERT(m_ptype);
  if( val.isEmpty() )
...
{
#ifdef ES_DEBUG
  EsRttiPropertyIndexLink* il =
#endif
  new EsRttiPropertyIndexLink(this, index, cmp, rttiProp);
...
void EsRttiPropertyLinkIndexed::linkAdd(EsRttiPropertyIndexLink* link)
{
  ES_ASSERT(link);
...
void EsRttiPropertyLinkIndexed::linkRemove(EsRttiPropertyIndexLink* link)
{
  ES_ASSERT(link);
...
//
__fastcall EsSpinCtlPropertyLink::EsSpinCtlPropertyLink(const EsString& propName, TCustomEditBox* ctl, TLabel* lbl) :
...
void EsSpinCtlPropertyLink::initialize()
{
  ...
  m_ctl->Hint = EsStringI18n::translate( description.c_str() ).c_str();
  m_ctl->ValueType = TNumValueType::Integer;
  ...
  if( !restriction.isEmpty() )
  {
    ...
    if( min.isEmpty() )
    {
      m_ctl->Min = std::numeric_limits<int64_t>::min();
      m_ctl->Max = max.asLLong();
    }
    else if( max.isEmpty() )
    {
      m_ctl->Max = std::numeric_limits<int64_t>::max();
      m_ctl->Min = min.asLLong();
    }
    else
    {
      m_ctl->Min = min.asLLong();
...
{
  m_ctl->Value = val.asLLong();
}
//---------------------------------------------------------------------------

EsVariant EsSpinCtlPropertyLink::doControlValGet() const
{
  return static_cast<llong>(m_ctl->Value);
}
//---------------------------------------------------------------------------
//---------------------------------------------------------------------------
...
void EsSpinCtlDoublePropertyLink::initialize()
{
  ...
  m_ctl->Hint = EsStringI18n::translate( description.c_str() ).c_str();
  m_ctl->ValueType = TNumValueType::Float;
...
///
__fastcall EsTimeEditPropertyLink::EsTimeEditPropertyLink(const EsString& propName, TCustomTimeEdit* ctl, TLabel* lbl) :
EsReflectedClassPropertyLink(ctl, propName),
m_ctl(ctl),
m_lbl(lbl),
m_minAssigned(false),
...
__fastcall EsTimeEditPropertyLink::~EsTimeEditPropertyLink()
{}
//---------------------------------------------------------------------------

bool EsTimeEditPropertyLink::isUnidirectional() const
{
  return !m_ctl->Enabled;
}
//---------------------------------------------------------------------------

void __fastcall EsTimeEditPropertyLink::onI18nStringsUpdate(TObject* sender)
{
  initialize();
}
//---------------------------------------------------------------------------

void EsTimeEditPropertyLink::initialize()
{
  if( m_lbl )
  {
    const EsString& txt = EsStringI18n::translate( label.c_str() ) + esT(":");
...
bool EsTimeEditPropertyLink::valueRestrict(EsDateTime& dt) const
{
  bool result = false;
  ...
  ES_DEBUG_TRACE(
    esT("EsTimeEditPropertyLink::valueRestrict(%s)->%d"),
    dt.asString(),
    (int)result
  );
...
bool EsTimeEditPropertyLink::doValidateControlVal(const EsVariant& val, EsVariant& suggested) const
{
  if( val.isEmpty() )
    return false;
  ...
  if( valueRestrict(dt) )
  {
    suggested = EsDateTime::toVariant(dt);
    return false;
  }
...
void EsTimeEditPropertyLink::doControlValSet(const EsVariant& val)
{
  EsDateTime dt;

  if( !val.isEmpty() )
    dt = EsDateTime::fromVariant( val );

  valueRestrict(dt);

  TDateTime tdt = EsUtilities::toTDateTime( dt );

  ES_DEBUG_TRACE(
    esT("EsTimeEditPropertyLink::doControlValSet(%s)"),
    esVS(tdt.DateTimeString())
  );
...
EsVariant EsTimeEditPropertyLink::doControlValGet() const
{
  unsigned short h, m, s, ms;
  m_ctl->Time.DecodeTime(
    &h,
    &m,
    &s,
    &ms
  ...
  ES_DEBUG_TRACE(
    esT("EsTimeEditPropertyLink::doControlValGet()->%s"),
    dt.asString()
  );

  return EsDateTime::toVariant(
    dt
  );
}
//---------------------------------------------------------------------------
